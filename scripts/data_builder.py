...
if __name__ == '__main__':
    # path = 'data/composit3.json'
    # new_path, data = extract_context(path)
    # print(new_path)
    # check_alignment(new_path, verbose=True)
    # path = 'data/composit3.json'
    # print(check_alignment(path, verbose=True))
    with open('data/names6.json', 'r') as f:
        lines = json.load(f)
        data = []
        for line in lines:
            data.append(replaceAndLabel(line['message']))
        with open('data/spancat7.json', 'w') as j:
            json.dump(data, j, ensure_ascii=False, indent=1, separators=(',', ': '))
